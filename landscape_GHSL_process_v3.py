...
fun_names = {
    ...
    'F': 'F - małomiasteczkowe i zurbanizowane',
    'G': 'G - wiejskie'}
...
if plot_charts_types:
    ...
    for idx, fun_group in enumerate(fun_groups):
        ...
        # Filter municipalities belonging to this functional group
        ...
        for krajobraz in agg_df['nazwa']:
            ...
            if pop_vals[0] == 0:
                continue
            # Consider only polygons that had population in 2020 > 1000
        ...
        # Sort by 2020 relative value
        plot_data_sorted = sorted(plot_data, key=lambda x: x["pop_2020_rel"], reverse=True)

        # Plot lines
        for data in plot_data_sorted:
            color = color_dict.get(data["krajobraz"], "#000000")

            line, = ax.plot(years, data["pop_rel"], color=color, lw=2, label=data["krajobraz"])
        ...
        # Axes labels only on left and bottom subplots
        if idx // 2 == 1:  # bottom row
            ax.set_xlabel("Rok")

        if idx % 2 == 0:  # left column
            ax.set_ylabel("Ludność względna (1975 = 100)")
...
if plot_charts_types_agg:
    ...
    # Extract class Rychlinga = first digit
    ...
    for idx, fun_group in enumerate(fun_groups):
        ...
        # Aggregate population by class instead of by landscape type
        agg_dict = {f'pop_{y}': 'sum' for y in years}
        agg_df = result_df.groupby('klasa').agg(agg_dict).reset_index()

        # Prepare data for 4 classes
        ...
        group_label = "\n".join([fun_names[x] for x in fun_group])
        ax.set_title(f"{group_label}")
        ...
        # if idx % 2 == 0:
        #     ax.set_ylabel("Względna liczba ludności (1975 = 100)")
        ax.set_xlabel(" ")
        if idx==0: ax.set_ylabel("Względna liczba ludności (1975 = 100)")
    ...
    # Legend below
    fig.legend(
        handles=[lh[0],lh[1],lh[3],lh[2]],
        labels=[ll[0],ll[1],ll[3],ll[2]],
    ...
    plt.tight_layout(rect=[0, 0, 0.88, 1])
    fig.savefig(
        datadir + os.sep + f"relative_population_by_functional_groups_CLASSES_{version}.png",
        dpi=300,
        bbox_inches="tight"
    )
    plt.show()
